...
/**
 * cp oldpath newpath: Moves item from oldpath to newpath, 
 * recursively copying the contents.
 */
public class cp extends Command {
  ...
  public void execute(String[] args) {
    if (args.length == 2) {
      DefaultMutableTreeNode oldNode = copy(getNode(args[0]));
  ...
  /**
   * Recursively creates a copy of a tree.
   * @param node The node to copy.
   * @return Returns the completed copy.
   */
  public DefaultMutableTreeNode copy(DefaultMutableTreeNode node) {
    ...
    for (int i = 0; i < node.getChildCount(); i++) {
      newNode.add(copy((DefaultMutableTreeNode) node.getChildAt(i)));
    }
    return newNode;
  }
  /**
